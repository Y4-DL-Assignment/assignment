...
class DQNAgent:
    def __init__(self, env=None, state_dim=None, num_actions=4, lr=0.001,
                 gamma=0.95, buffer_capacity=100000, batch_size=128, tau=0.005):
        """
        Deep Q-Learning Agent with soft target network updates.
        
        Args:
            env: GridWorld environment (to infer state/action dimensions)
            state_dim: Manual state dimension (if env not provided)
        ...
        if state_dim is None:
            raise ValueError("Must provide either env or state_dim")
