...
class GridWorldEnv:
    ...
    - 10×10 grid
    - Goal respawns after being reached (like food in Snake)
    - Obstacles move every 3 steps (dynamic environment)
    - Bomb stays fixed until episode ends (permanent hazard)
    - Episode only ends on collision or timeout
    - Distance-based reward shaping (like Snake)
    """

    def __init__(self, grid_size=10, max_steps=1000, use_obstacles=True, fixed_start=True, obstacle_move_freq=3):
        self.n = grid_size
        self.max_steps = max_steps
        self.use_obstacles = use_obstacles
        self.fixed_start = fixed_start
        self.obstacle_move_freq = obstacle_move_freq  # Obstacles move every N steps
        self.ACTIONS = {
            0: (-1, 0),  # UP
            1: (1, 0),   # DOWN
            2: (0, -1),  # LEFT
    ...
    def reset(self):
        ...
        self.score = 0
        self.steps_without_goal = 0
        self.steps_since_obstacle_move = 0  # Track steps for obstacle movement
        forbidden = set()
        ...
        # Obstacles (will respawn periodically)
        self.obstacles = set()
        if self.use_obstacles:
            for _ in range(2):
                cell = self._rand_free_cell(forbidden)
    ...
    def _respawn_obstacles_only(self):
        """
        Respawn obstacles at new random locations.
        Called every N steps to make environment dynamic (like Snake's tail moving).
        Goal and bomb stay at same location.
        """
        forbidden = {self.agent, self.bomb, self.goal}
        ...
        # Update obstacle distance tracking
        self.prev_obstacle_distance = self._get_min_distance_to_obstacles()

    def _respawn_goal_and_obstacles(self):
        """
        Respawn goal and obstacles at new locations (when goal is reached).
        Bomb stays at the same location.
        """
        forbidden = {self.agent, self.bomb}
        ...
        # Reset obstacle movement counter
    ...
    def step(self, action):
        ...
        # MOVE OBSTACLES EVERY N STEPS (like Snake's tail moving periodically)
        if self.steps_since_obstacle_move >= self.obstacle_move_freq:
            self._respawn_obstacles_only()
            self.steps_since_obstacle_move = 0

        # Check obstacle collision AFTER potential obstacle movement (episode ends)
        if self.agent in self.obstacles:
            return StepResult(self._get_obs(), -10.0, True, {"reason": "obstacle"})
    ...
    def render(self):
        ...
        print(f"Score: {self.score} | Step: {self.t} | "
              f"Steps without goal: {self.steps_without_goal}/{self.max_steps} | "
              f"Obstacle moves in: {self.obstacle_move_freq - self.steps_since_obstacle_move} steps\n")
...
#             # NEW: Oscillation tracking
#         self.position_history = []
#         self.oscillation_threshold = 4  # Trigger if same position 4+ times in last 10 steps
...
#         """
#         NEW: Check if agent is oscillating (visiting same positions repeatedly).
#         Returns True if oscillation detected.
#         """
#         if len(self.position_history) < 10:
#             return False
        
#         # Count how many times current position appears in last 10 steps
#         current_pos = self.agent
#         recent_positions = self.position_history[-10:]
#         count = recent_positions.count(current_pos)
        
#         # Also check if alternating between 2 positions
#         if len(recent_positions) >= 6:
#             last_6 = recent_positions[-6:]
#             if len(set(last_6)) == 2:  # Only 2 unique positions in last 6 steps
#                 return True
        
#         return count >= self.oscillation_threshold
...
#         # NEW: Track position for oscillation detection
#         self.position_history.append(self.agent)
...
#         # BOMB: Penalty for moving closer, reward for moving away
#         if current_bomb_distance < self.prev_bomb_distance:
#             # Moving closer to bomb - penalty scales with proximity
#             danger_multiplier = max(1.0, 5.0 / (current_bomb_distance + 1))
#             reward -= 0.8 * danger_multiplier
#         else:
#             reward += 0.5  # Moving away from bomb is safe (POSITIVE)

#         # SPECIAL: Very close to bomb (within 2 cells)
#         if current_bomb_distance <= 2:
#             reward -= 2.5  # STRONG penalty when very close to bomb!
...
#             reward -= 4.0  # STRONG penalty for oscillating
#             # Optional: Print for debugging
#             # print(f"⚠ Oscillation detected at {self.agent} (step {self.t})! Penalty: -4.0")
