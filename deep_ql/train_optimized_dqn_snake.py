...
# Device selection (M4 MPS support)
if torch.backends.mps.is_available():
    DEVICE = torch.device('mps')
    print("✓ Using MPS (Metal Performance Shaders) on M4")
else:
...
class DQNAgent:
    def __init__(self, state_size=16, action_size=3, learning_rate=0.0005,
        ...
        # Main and target networks on device
        ...
        self.criterion = nn.MSELoss()  # OPTIMIZATION: Changed from SmoothL1Loss to MSE
        self.update_counter = 0
        self.step_counter = 0  # Track steps for replay frequency
    ...
    def replay(self, batch_size=64):  # OPTIMIZATION: Reduced batch size from 128 to 64
...
def train_agent(training_env, training_agent, episodes=50000):
    ...
    step_counter = 0  # OPTIMIZATION: Track steps for replay frequency

    print(f"Training for {episodes} episodes with GPU acceleration...")
    ...
    for episode in range(episodes):
        ...
        while not training_env.done:
            ...
            # OPTIMIZATION: Only replay every 4 steps instead of every step
            if step_counter % 4 == 0:
...
class SnakeVisualizer(tk.Tk):
    def __init__(self, env, agent):
        ...
        self.env = env
        self.agent = agent
        self.title("Snake Game - DQN (Optimized)")
        ...
        # Device label
        device_text = "GPU (MPS)" if DEVICE.type == 'mps' else "CPU"
        self.device_label = tk.Label(self.info_frame, text=f"Device: {device_text}",
                                     font=("Arial", 12), fg="green" if DEVICE.type == 'mps' else "orange")
        self.device_label.pack(side=tk.LEFT, padx=10)

        # Input features and output decision widgets
    ...
    def draw_game(self):
        ...
        # Update input features and Q-values display
        ...
        try:
            with torch.no_grad():
                q_tensor = self.agent.model(torch.FloatTensor(state).unsqueeze(0).to(DEVICE))
            q_values = q_tensor.squeeze().detach().cpu().numpy()
        except Exception:
...
if __name__ == "__main__":
    ...
    try:
        ...
        torch.onnx.export(agent.model, dummy_input, "./dqn_snake.onnx",
