...
class AcademicNetworkVisualizer(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Deep Q-Network Architecture Analysis")
        self.geometry("1400x900")
        self.configure(bg="#f5f5f5")

        # Initialize model
        self.model = DQN(input_size=16, hidden_size=256, output_size=3)
        self.model.eval()
        ...
        try_paths = [
            os.path.join(os.path.dirname(__file__), 'improved_dqn_snake_10x10.pth'),
            os.path.join(os.path.dirname(__file__), 'improved_dqn_snake_10x10.pt'),
            'improved_dqn_snake_10x10.pth',
            os.path.join('deep_ql', 'improved_dqn_snake_10x10.pth'),
            os.path.join('.', 'deep_ql', 'improved_dqn_snake_10x10.pth')
        ]
        for p in try_paths:
            try:
    ...
    def _setup_header(self):
        ...
        title = tk.Label(
            header,
            text="Deep Q-Network (DQN) Architecture Analysis",
